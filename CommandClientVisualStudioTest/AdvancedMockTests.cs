...
namespace CommandClientVisualStudioTest
{
    ...
    public class AdvancedMockTests
    {
        ...
        public void TestUserExitCommandWithoutMocks()
        {
            ...
            byte[] merge = {0,0,0,0,9,0,0,0,49,50,55,46,48,46,48,46,49,2,0,0,0,10,0};


            CMDClient client = new CMDClient(null, "Bogus network name");
            typeof(CMDClient).GetField("networkStream", BindingFlags.NonPublic | BindingFlags.Instance).SetValue(client, memoryStream);
            client.SendCommandToServerUnthreaded(command);
           
           CollectionAssert.AreEqual(merge,memoryStream.ToArray());
        }
